        ...
        acceptedDemandRepository.exist(demand.id)
            ...
            .flatMap { isExists ->
                if(isExists == beExists) Unit.toMono()
                    ...
                    logger.warn("이미 ACCEPT/REJECT된 예약에 대한 ACCEPT Operation이 실행되었습니다.")
                    return@flatMap Mono.error(AlreadyRejectedDemandException("수락하거나 거절하신 예약은 재수락하실 수 없습니다! - ${demand.id}"))
                }
            }
}