...
class DemandPolicyValidatorImplTest {
    ...
    @Test @DisplayName("IP할당신청취소정책 검사 성공 테스트")
    fun testValidateCancelSuccess() {
        //취소정책 검사에 성공하였다면, 신청정보를 반환한다.
        //취소정책 검사에 성공하려면 아래의 작업이 성공해야한다.
        //- 신청취소정책 검사 트랜잭션 수행

        //신청취소정책 검사 트랜잭션이 성공하려면, 아래의 조건을 만족해야한다.
        //- 신청이 존재할 것
        //- issuer가 신청의 issuer(소유자)일 것
    ...
    @Test @DisplayName("IP할당신청추가정책 검사 성공 테스트")
    fun testValidateCreateSuccess() {
        //추가정책 검사에 성공하였다면, 신청정보를 반환한다.
        //추가정책 검사에 성공하려면 아래의 작업이 성공해야한다.
        //- 신청추가정책 검사 트랜잭션 수행

        //신청추가정책 검사 트랜잭션이 성공하려면, 아래의 조건을 만족해야한다.
        //- expireAt이 오늘 이후일 것
        //- title이 25자 이하일 것
    ...
    //TODO 신청거절정책에 수락된 신청에 존재하지 않을 것 조건추가하기
    @Test @DisplayName("IP할당신청거절정책 검사 성공 테스트")
    fun testValidateRejectSuccess() {
        //거절정책 검사에 성공하였다면, 신청정보를 반환한다.
        //거절정책 검사에 성공하려면 아래의 작업이 성공해야한다.
        //- 신청거절정책 검사 트랜잭션 수행

        //신청거절정책 검사 트랜잭션이 성공하려면, 아래의 조건을 만족해야한다.
        //- 신청이 존재할 것
    ...
    //TODO 신청수락정책에 거절된 신청에 존재하지 않을 것 조건추가하기
    ...
    fun testValidateAcceptSuccess() {
        ...
        //신청수락정책 검사 트랜잭션이 성공하려면, 아래의 조건을 만족해야한다.
        //- 신청이 존재할 것
